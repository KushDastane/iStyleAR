...
const CreativeCarousel = forwardRef(({
  ...
  useImperativeHandle(ref, () => ({
    slideToItem: (index) => {
      if (swiperRef.current) {
        // Slide to the specific item index directly
        // Swiper handles snapping to the nearest group if slidesPerGroup is set
  ...
  useEffect(() => {
    ...
    if (index > -1) {
      // Just slide to the item index directly, let Swiper handle group alignment
      swiperRef.current.slideTo(index);
    }
              ...
              >
                ...
                >
                  <img
                    src={item.imageUrl}
                    alt={item.name}
                    className={`max-h-full max-w-full object-contain transition-all duration-500 ${isHovered ? "scale-110" : "scale-100"
                      }`}
                  />

                  <div
                    className={`absolute inset-0 bg-gradient-to-t from-black/10 to-transparent transition-opacity duration-300 ${isHovered ? "opacity-100" : "opacity-0"
                      }`}
                  />
                </div>
                ...
                {!hideNames && (
                  <div
                    className={`flex flex-col p-4 justify-between bg-white ${hasButton ? "flex-grow" : "flex-none"
                      }`}
                  >
                    <div className="flex-grow">
                      <h3
                        className={`font-semibold text-center text-gray-900 line-clamp-2 leading-snug ${hasButton ? "text-base mb-3" : "text-base mb-2"
                          }`}
                      >
                        {item.name}
                      </h3>
